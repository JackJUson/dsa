...
static BSTree newNode(int key);
static void doBSTreeShow(BSTree t);
...
BSTree BSTreeInsert(BSTree t, int key) {
    if (t == NULL) {
        t = newNode(key);
    } else if (key < t->key) {
        t->left = BSTreeInsert(t->left, key);
    } else if (key > t->key) {
        t->right = BSTreeInsert(t->right, key);
    }
    return t;
}
